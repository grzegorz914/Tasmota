 ...
 * distinct, reliable climate fan mode. AirDirection
 * presets are exposed only after a vertical vane and i-See support are known.
...
miel_hvac_hass_discovery(struct miel_hvac_softc *sc)
{
	...
	const char *fan_fallback_state = fan_auto_capable ? "Auto" : "1";
	ResponseAppend_P(PSTR("%s\","
		...
		fan_auto_state, fan_fallback_state);
